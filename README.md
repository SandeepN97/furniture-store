...
If an item is out of stock the order will still be accepted and marked with the `BACKORDER` status. Stock is not decremented until the item ships.

## Shipping Rates

`GET /api/shipping/rate?provider=UPS&weight=5` returns a basic shipping cost used during checkout. When updating an order status, admins may set a tracking number and provider.

## Returns and Refunds

Customers can POST to `/api/returns` with the order ID and a reason to request a return. Admins view requests and issue refunds via `/api/returns/{id}/refund`.

## Pickup or Delivery

At checkout customers choose **Delivery** or **Pickup**. The selected method is stored with the order.

## Bulk Product Upload

Admins may upload a CSV file to `/api/products/upload-csv` where each line is: `name,price,description,categoryId,stockQuantity` to create many products at once.

## QR Inventory Scanning

Admins can navigate to `/scan` to open a camera-based QR code scanner. Scanning a code with a product ID displays its current stock quantity for quick warehouse checks.

## Voice Navigation

Click the microphone button in the bottom left to issue commands like "go to cart" or "open blog". Supported actions navigate around the app using the Web Speech API.

## Room Designer

Visit `/room-designer` to open a simple drag-and-drop canvas where you can place sample sofas, tables and lamps around a virtual room.

## AI Assistant

The page at `/assistant` lets you chat with our AI consultant about furniture. Enter any question like "Which sofa should I buy?" and it will respond using OpenAI.
