        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {products.map((p) => (
            ...
            >
              ...
              <p className="text-sm">{p.stockQuantity > 0 ? `Stock: ${p.stockQuantity}` : t('outOfStock')}</p>
              <button
                onClick={() => addItem(p)}
                disabled={p.stockQuantity === 0}
                className="mt-2 bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 disabled:opacity-50"
              >
                {t('addToCart')}
              </button>
            </div>
